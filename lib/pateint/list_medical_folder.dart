...
class MedicalFolderPatient extends StatefulWidget {
  MedicalFolderPatient({super.key});

  @override
  State<MedicalFolderPatient> createState() => _MedicalFolderPatientState();
}

class _MedicalFolderPatientState extends State<MedicalFolderPatient> {
...
class _ListRecordsState extends State<ListRecords> {
  ...
  void fetchRecords() async {
    var url = Uri.parse('http://${server}:8000/folder/my_records/');
                    ...
                    Row(
                      children: [
                        InkWell(
                          ...
                          onTap: () async {
                            var url = Uri.parse(
                                'http://${server}:8000/folder/my_records/');
...
class _ReportsState extends State<Reports> {
  ...
  void fetchReports() async {
    var url = Uri.parse('http://${server}:8000/folder/my_reports/');
    final response = await get(
      url,
      headers: {
        'Content-Type': 'application/json; charset=UTF-8',
                      ...
                      Column(
                        children: [
                          ...
                          InkWell(
                            ...
                            onTap: () async {
                              var url = Uri.parse(
                                  'http://${server}:8000/folder/my_reports/');
                              final response = await delete(url,
                                  headers: {
                                    'Content-Type':
                                        'application/json; charset=UTF-8',
